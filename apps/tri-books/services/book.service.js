...
const gBooks = storageService.loadFromStorage('BooksDB') || [
    {
        ...
        "review": [_createReview('Muki ben Puki', 4, '10/09/2020', 'wow, thats a great book! im gonna read it again'),
        _createReview('Shuki laka boom', 1, '25/07/2020', 'Worst Book EVAHHHHHHH, cant stand it!'),
        _createReview('Nana Banana', 5, '13/01/2021', 'Finish it in one day!!! wow!!!!!!!!!!!!!!!!!!')]
    },
    {
        "id": "JYOJa2NpSCq",
        "title": "morbi",
...
function addBook(googleBook) {
    const isBookExists = gBooks.find(book => book.id === googleBook.id)
    if (isBookExists) return isBookExists;
    const book = {
        "id": googleBook.id,
        "title": googleBook.volumeInfo.title,
        "subtitle": googleBook.volumeInfo.subtitle || '',
        "authors": googleBook.volumeInfo.authors,
        "publishedDate": googleBook.volumeInfo.publishedDate.substring(0, 4),
...
function addReview(fullName = "Books Reader", rate = 0, readAt, txt = "No review text provided", bookId) {
    const book = gBooks.find(book => book.id === bookId);
    const review = _createReview(fullName, rate, readAt, txt)
    if (!book.review) book.review = [];
    book.review.unshift(review);
    storageService.saveToStorage('BooksDB', gBooks);
}

function _createReview(fullName, rate, readAt, txt) {
    return {
        fullName,
        rate,
        readAt,
